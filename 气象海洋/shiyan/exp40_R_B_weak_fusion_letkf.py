...
OUTPUT_DIR = ROOT / "exp40_R_B_weak_fusion_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "R_B_weak_fusion_letkf"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验40：R-B 弱融合 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None, float | None]]:
    ...
    for lam_r, lam_b in LAMBDA_COMBOS:
        experiments.append(("r_b_weak_fusion", lam_r, lam_b))
    return experiments


def result_file_stem(method: str, lam_r: float | None, lam_b: float | None) -> str:
    if lam_r is None and lam_b is None:
        return method
    return (
        f"{method}_r_{str(lam_r).replace('.', '_')}_"
...
def main() -> None:
    ...
    for method, lam_r, lam_b in experiments:
        ...
        for split, path in datasets:
            ...
            out_path = args.output_dir / f"{split}_{result_file_stem(method, lam_r, lam_b)}.csv"
