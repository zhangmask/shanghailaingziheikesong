...
OUTPUT_DIR = ROOT / "exp18_quantum_J_operator_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "quantum_J_operator_letkf"
...
def project_to_psd_correlation(matrix: np.ndarray) -> np.ndarray:
    sym = 0.5 * (matrix + matrix.T)
    diag = np.clip(np.diag(sym), EPS, None)
    corr = sym / np.outer(np.sqrt(diag), np.sqrt(diag))
    corr = 0.5 * (corr + corr.T)
    vals, vecs = np.linalg.eigh(corr)
    vals = np.clip(vals, 1e-4, None)
    corr = vecs @ np.diag(vals) @ vecs.T
    corr = 0.5 * (corr + corr.T)
    d = np.sqrt(np.clip(np.diag(corr), EPS, None))
    corr = corr / np.outer(d, d)
    return 0.5 * (corr + corr.T)


def build_quantum_geometries(
    i: int,
    x_mean: np.ndarray,
    y_pert: np.ndarray,
    obs_vec: np.ndarray,
    ...
    q_weights = normalize_weights(build_quantum_lookup(state_window, obs_windows)[0], min_clip=0.1)

    prior_mode = y_pert @ q_weights
    prior_mode = prior_mode - prior_mode.mean()
    prior_norm = np.linalg.norm(prior_mode) + EPS
    prior_mode = prior_mode / prior_norm

    prior_precision = ((ENS_SIZE - 1) / INFL) * (
        np.eye(ENS_SIZE, dtype=float) + lam * np.outer(prior_mode, prior_mode)
    )

    corr = project_to_psd_correlation(similarity)
    residual_metric = OBS_VAR * ((1.0 - lam) * np.eye(corr.shape[0], dtype=float) + lam * corr)
    residual_metric = 0.5 * (residual_metric + residual_metric.T)
    residual_metric += 1e-6 * OBS_VAR * np.eye(corr.shape[0], dtype=float)
    return prior_precision, residual_metric
...
def letkf_update_weighted(
    ...
    if len(obs_idx_all) == 0:
        return ensemble.copy(), {"avg_quantum_prior_trace": 0.0}

    global_obs_similarity = None
    if method == "quantum_j_operator":
        obs_fill = np.nan_to_num(obs_vec, nan=0.0)
    ...
    for i in range(nx):
        ...
        prior_precision, residual_metric = build_quantum_geometries(
            i=i,
            x_mean=x_mean,
            y_pert=y_pert,
            obs_vec=obs_vec,
        ...
        m_inv = stable_inverse(residual_metric)
        c_mat = prior_precision + y_pert @ m_inv @ y_pert.T
        pa_tilde = stable_inverse(c_mat)
        w_mean = pa_tilde @ y_pert @ m_inv @ innovation
        w_pert = sym_sqrt((nens - 1) * pa_tilde)
        x_mean_a = x_mean[i] + x_pert[:, i] @ w_mean
        x_pert_a = x_pert[:, i] @ w_pert
        ana[:, i] = x_mean_a + x_pert_a
        q_prior_total += float(np.trace(prior_precision))
        q_steps += 1

    return ana, {"avg_quantum_prior_trace": float(q_prior_total / max(q_steps, 1))}
...
def run_filter(
    ...
    stats_acc = {"avg_quantum_prior_trace": 0.0}
    stats_steps = 0
    ...
    if np.any(~np.isnan(obs[0])):
        ...
        analysis[0] = ensemble.mean(axis=0)
        if method == "quantum_j_operator":
            stats_acc["avg_quantum_prior_trace"] += step_stats["avg_quantum_prior_trace"]
            stats_steps += 1
    ...
    if stats_steps > 0:
        stats_acc["avg_quantum_prior_trace"] /= stats_steps
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验18：量子代价函数几何 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None]]:
    ...
    for lam in LAMBDAS:
        experiments.append(("quantum_j_operator", lam))
    return experiments
...
def main() -> None:
    ...
    for method, lam in experiments:
        ...
        for split, path in datasets:
            ...
            if method == "quantum_j_operator":
                rec[f"{split}_avg_quantum_prior_trace"] = stats["avg_quantum_prior_trace"]
    ...
    with (args.output_dir / "summary.csv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "train_mae",
                "train_count",
                "train_avg_quantum_prior_trace",
                "test_1_rmse",
                "test_1_mae",
                "test_1_count",
                "test_1_avg_quantum_prior_trace",
            ],
        )
        writer.writeheader()
