...
OUTPUT_DIR = ROOT / "exp6_quantum_credibility_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "quantum_credibility_features"
...
EPS = 1e-8
LAMBDAS = [0.02, 0.05, 0.08, 0.10, 0.12, 0.15]
FEATURE_NAMES = ["innovation_score", "spread_score", "coupling_score", "distance_score"]
...
def build_reference_state() -> Statevector:
    # 理想可信观测对应“高可信度四元特征全为 1”的锚点态。
    cached = getattr(build_reference_state, "_cached_state", None)
    if cached is None:
        reference_feature = np.full(N_QUBITS, FEATURE_SCALE, dtype=float)
        cached = Statevector.from_instruction(build_feature_map(reference_feature))
        build_reference_state._cached_state = cached
    return cached


def credibility_scores_for_observations(
    obs_idx_all: np.ndarray,
    x_mean: np.ndarray,
    x_pert: np.ndarray,
    obs_vec: np.ndarray,
) -> np.ndarray:
    obs_features = np.zeros((len(obs_idx_all), N_QUBITS), dtype=float)
    reference_state = build_reference_state()

    for pos, j in enumerate(obs_idx_all):
        obs_signal = x_pert[:, j]
        innovation_mag = abs(obs_vec[j] - x_mean[j]) / (OBS_STD + EPS)
        obs_std = obs_signal.std(ddof=1) + EPS
        spread_mag = obs_std / (OBS_STD + EPS)
        neighbor_window = cyclic_window(x_mean, int(j), N_QUBITS)
        neighbor_idx = [(int(j) + off) % len(x_mean) for off in range(-(N_QUBITS // 2), -(N_QUBITS // 2) + N_QUBITS)]
        neighbor_corrs: list[float] = []
        for neighbor in neighbor_idx:
            neighbor_signal = x_pert[:, neighbor]
            neighbor_std = neighbor_signal.std(ddof=1) + EPS
            corr = abs((neighbor_signal * obs_signal).mean() / (neighbor_std * obs_std))
            neighbor_corrs.append(float(np.clip(corr, 0.0, 1.0)))
        coupling_score = float(np.mean(neighbor_corrs))
        local_consistency = float(np.exp(-abs(obs_vec[j] - neighbor_window.mean()) / (OBS_STD + EPS)))

        innovation_score = float(np.exp(-innovation_mag))
        spread_score = float(1.0 / (1.0 + spread_mag))

        obs_features[pos] = np.array(
            [innovation_score, spread_score, coupling_score, local_consistency],
            dtype=float,
        )

    pair_angles = FEATURE_SCALE * np.clip(obs_features, 0.0, 1.0)
    credibility = np.zeros(len(obs_idx_all), dtype=float)
    for pos, feature in enumerate(pair_angles):
        obs_state = Statevector.from_instruction(build_feature_map(feature))
        overlap = np.vdot(obs_state.data, reference_state.data)
        credibility[pos] = float(np.abs(overlap) ** 2)
    return credibility


def compute_quantum_lookup_for_step(x_mean: np.ndarray, x_pert: np.ndarray, obs_vec: np.ndarray, obs_idx_all: np.ndarray) -> np.ndarray:
    return credibility_scores_for_observations(
...
def compute_weights(
    ...
    if quantum_lookup is None or lam is None:
        raise ValueError("Credibility quantum method requires quantum_lookup and lambda")

    if method == "credibility_quantum_mix":
        local_pos = np.searchsorted(obs_idx_all, local_obs_idx)
        quantum_raw = np.clip(quantum_lookup[local_pos], 0.0, None)
...
def letkf_update_weighted(
    ...
    quantum_lookup = None
    if method == "credibility_quantum_mix":
        quantum_lookup = compute_quantum_lookup_for_step(
            x_mean=x_mean,
            x_pert=x_pert,
            obs_vec=obs_vec,
            obs_idx_all=obs_idx_all,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验6：量子可信度特征映射 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None]]:
    ...
    for lam in LAMBDAS:
        experiments.append(("credibility_quantum_mix", lam))
    return experiments
