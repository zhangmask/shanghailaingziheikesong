...
OUTPUT_DIR = ROOT / "exp8_quantum_block_mix_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "quantum_block_mix_letkf"
...
def build_local_blocks(local_obs_idx: np.ndarray, block_size: int) -> list[np.ndarray]:
    if len(local_obs_idx) == 0:
        return []
    return [local_obs_idx[start : start + block_size] for start in range(0, len(local_obs_idx), block_size)]


def compute_block_quantum_weights(
    i: int,
    local_obs_idx: np.ndarray,
    obs_idx_all: np.ndarray,
    nx: int,
    support_radius: int,
    quantum_lookup: np.ndarray,
    lam: float,
) -> np.ndarray:
    local_pos = np.searchsorted(obs_idx_all, local_obs_idx)
    quantum_raw = np.clip(quantum_lookup[i, local_pos], 0.0, None)
    block_weights = np.zeros(len(local_obs_idx), dtype=float)
    blocks = build_local_blocks(local_obs_idx, BLOCK_SIZE)
    cursor = 0

    # 先在 block 内聚合量子信号，再回写到观测点层，避免单点量子权重过于尖锐。
    for block in blocks:
        block_len = len(block)
        block_slice = slice(cursor, cursor + block_len)
        block_signal = float(np.mean(quantum_raw[block_slice]))
        block_weights[block_slice] = (1.0 - lam) + lam * block_signal
        cursor += block_len

    dist_w = distance_weight(i=i, local_obs_idx=local_obs_idx, nx=nx, support_radius=support_radius)
    return normalize_weights(dist_w * block_weights, min_clip=0.2)
...
def compute_weights(
    ...
    if method == "block_quantum_mix":
        if quantum_lookup is None or lam is None:
            raise ValueError("Block quantum mix requires quantum_lookup and lambda")
        return compute_block_quantum_weights(
            i=i,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验8：量子 block 弱混合 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None]]:
    ...
    for lam in LAMBDAS:
        experiments.append(("block_quantum_mix", lam))
    return experiments
