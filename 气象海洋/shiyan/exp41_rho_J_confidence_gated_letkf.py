...
OUTPUT_DIR = ROOT / "exp41_rho_J_confidence_gated_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "rho_J_confidence_gated_letkf"
...
def build_j_gated_state_signal(
    ...
    return corrected, correction_strength, j_confidence
...
def letkf_update_weighted(
    ...
    if len(obs_idx_all) == 0:
        return ensemble.copy(), {"avg_j_correction_strength": 0.0, "avg_j_confidence": 0.0}
    ...
    for i in range(nx):
        ...
        if method != "rho_j_confidence_gated":
        ...
        corrected_signal, strength, j_confidence = build_j_gated_state_signal(
            i=i,
            x_pert=x_pert,
            y_pert=y_pert,
        ...
        confidence_total += j_confidence
        correction_steps += 1

    stats = {
        "avg_j_correction_strength": float(correction_total / max(correction_steps, 1)),
        "avg_j_confidence": float(confidence_total / max(correction_steps, 1)),
    }
    return ana, stats
...
def run_filter(
    ...
    stats_acc = {"avg_j_correction_strength": 0.0, "avg_j_confidence": 0.0}
    stats_steps = 0
    ...
    if np.any(~np.isnan(obs[0])):
        ...
        if method == "rho_j_confidence_gated":
            ...
            stats_acc["avg_j_confidence"] += step_stats["avg_j_confidence"]
            stats_steps += 1
    ...
    if stats_steps > 0:
        ...
        stats_acc["avg_j_confidence"] /= stats_steps
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验41：rho+J 门控 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None, float | None]]:
    ...
    for lam_rho, lam_j in LAMBDA_COMBOS:
        experiments.append(("rho_j_confidence_gated", lam_rho, lam_j))
    return experiments
