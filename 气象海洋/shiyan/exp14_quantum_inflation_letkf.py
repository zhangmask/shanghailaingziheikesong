...
OUTPUT_DIR = ROOT / "exp14_quantum_inflation_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "quantum_inflation_letkf"
...
def compute_quantum_inflation(i: int, x_mean: np.ndarray, obs_vec: np.ndarray, local_obs_idx: np.ndarray, innovation: np.ndarray, y_pert: np.ndarray, infl: float, lam: float) -> float:
    ...
    factor = 1.0 + lam * (combined - 0.75)
    return float(np.clip(infl * factor, 0.95, 1.20))
...
def letkf_update_weighted(
    ...
    for i in range(nx):
        ...
        if method != "quantum_inflation":
            raise ValueError(f"Unknown method: {method}")

        infl_local = compute_quantum_inflation(
            i=i,
            x_mean=x_mean,
            obs_vec=obs_vec,
        ...
        r_inv = np.diag(np.ones(len(local_obs_idx), dtype=float) / obs_var)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验14：量子膨胀项 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None]]:
    ...
    for lam in LAMBDAS:
        experiments.append(("quantum_inflation", lam))
    return experiments
