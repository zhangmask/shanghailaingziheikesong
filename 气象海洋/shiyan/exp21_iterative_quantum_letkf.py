...
OUTPUT_DIR = ROOT / "exp21_iterative_quantum_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "iterative_quantum_letkf"
...
def compute_iterative_alpha(
    ...
    quantum_signal = float(np.mean(build_quantum_lookup(state_window, obs_windows)[0]))
    ...
    alpha = 0.45 + lam * (0.35 * quantum_signal + 0.20 * innovation_signal + 0.10 * spread_signal)
    return float(np.clip(alpha, 0.35, 0.95))


def single_pass_update(
    ensemble: np.ndarray,
    obs_vec: np.ndarray,
    obs_var: float,
    support_radius: int,
    ...
    if len(obs_idx_all) == 0:
        return ensemble.copy(), {"avg_alpha": 1.0}
    ...
    for i in range(nx):
        ...
        w_mean = pa_tilde @ y_pert @ r_inv @ innovation
        w_pert = sym_sqrt((nens - 1) * pa_tilde)
        candidate = (x_mean[i] + x_pert[:, i] @ w_mean) + x_pert[:, i] @ w_pert
        alpha = compute_iterative_alpha(
            i=i,
            x_mean=x_mean,
            obs_vec=obs_vec,
        ...
        ana[:, i] = ensemble[:, i] + alpha * (candidate - ensemble[:, i])
        alpha_total += alpha
        alpha_steps += 1

    return ana, {"avg_alpha": float(alpha_total / max(alpha_steps, 1))}
...
def run_filter(
    ...
    if np.any(~np.isnan(obs[0])):
        if method == "iterative_quantum":
            for _ in range(INNER_ITERS):
                ensemble, step_stats = single_pass_update(
                    ensemble=ensemble,
                    obs_vec=obs[0],
                    obs_var=OBS_VAR,
                ...
                stats_acc["avg_alpha"] += step_stats["avg_alpha"]
                stats_steps += 1
        else:
            ensemble, _ = single_pass_update(
                ensemble=ensemble,
                obs_vec=obs[0],
                obs_var=OBS_VAR,
                support_radius=SUPPORT_RADIUS,
    ...
    if stats_steps > 0:
        stats_acc["avg_alpha"] /= stats_steps
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验21：量子迭代更新 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None]]:
    ...
    for lam in LAMBDAS:
        experiments.append(("iterative_quantum", lam))
    return experiments
...
def main() -> None:
    ...
    for method, lam in experiments:
        rec: dict[str, float | int | str | None] = {
            ...
            "lambda": lam,
            "support_radius": SUPPORT_RADIUS,
            "inner_iters": INNER_ITERS if method == "iterative_quantum" else 1,
        }
        for split, path in datasets:
            ...
            if method == "iterative_quantum":
                rec[f"{split}_avg_alpha"] = stats["avg_alpha"]
