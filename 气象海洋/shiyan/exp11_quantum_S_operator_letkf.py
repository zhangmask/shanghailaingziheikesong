...
OUTPUT_DIR = ROOT / "exp11_quantum_S_operator_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "quantum_S_operator_letkf"
...
def letkf_update_weighted(
    ...
    for i in range(nx):
        ...
        similarity = project_to_psd(global_obs_similarity[np.ix_(local_pos, local_pos)])
        classical_r_inv = np.eye(len(local_obs_idx), dtype=float) / obs_var
        classical_operator = y_pert @ classical_r_inv @ y_pert.T
        quantum_obs_operator = similarity / obs_var
        quantum_operator = y_pert @ quantum_obs_operator @ y_pert.T
        beta = float(np.trace(classical_operator) / (np.trace(quantum_operator) + EPS))
        rhs_classical = y_pert @ classical_r_inv @ innovation
        rhs_quantum = y_pert @ quantum_obs_operator @ innovation
        mixed_operator = (1.0 - float(lam)) * classical_operator + float(lam) * beta * quantum_operator
        mixed_rhs = (1.0 - float(lam)) * rhs_classical + float(lam) * beta * rhs_quantum
        c_mat = ((nens - 1) / infl) * np.eye(nens) + mixed_operator
        pa_tilde = stable_inverse(c_mat)
        w_mean = pa_tilde @ mixed_rhs
        w_pert = sym_sqrt((nens - 1) * pa_tilde)
        x_mean_a = x_mean[i] + x_pert[:, i] @ w_mean
        x_pert_a = x_pert[:, i] @ w_pert
        ana[:, i] = x_mean_a + x_pert_a
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验11：量子分析空间算子 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None]]:
    ...
    for lam in LAMBDAS:
        experiments.append(("quantum_s_operator", lam))
    return experiments
