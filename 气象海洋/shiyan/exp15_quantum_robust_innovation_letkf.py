...
OUTPUT_DIR = ROOT / "exp15_quantum_robust_innovation_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "quantum_robust_innovation_letkf"
...
def compute_quantum_robust_weights(i: int, x_mean: np.ndarray, obs_vec: np.ndarray, local_obs_idx: np.ndarray, innovation: np.ndarray, lam: float) -> np.ndarray:
    ...
    delta = OBS_STD * (1.0 + lam * np.clip(quantum_raw, 0.0, 1.0))
    scale = np.maximum(1.0, np.abs(innovation) / (delta + EPS))
    robust = 1.0 / scale
    return normalize_weights(robust, min_clip=0.10)
...
def letkf_update_weighted(
    ...
    for i in range(nx):
        ...
        if method != "quantum_robust_innovation":
            raise ValueError(f"Unknown method: {method}")

        weights = compute_quantum_robust_weights(
            i=i,
            x_mean=x_mean,
            obs_vec=obs_vec,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验15：量子鲁棒创新度量 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None]]:
    ...
    for lam in LAMBDAS:
        experiments.append(("quantum_robust_innovation", lam))
    return experiments
