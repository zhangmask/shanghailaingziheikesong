...
OUTPUT_DIR = ROOT / "exp16_quantum_transform_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "quantum_transform_letkf"
...
def build_quantum_transform(
    pa_tilde: np.ndarray,
    similarity: np.ndarray,
    lam: float,
) -> tuple[np.ndarray, float]:
    classical_cov = (ENS_SIZE - 1) * pa_tilde
    vals, vecs = np.linalg.eigh(0.5 * (classical_cov + classical_cov.T))
    vals = np.clip(vals, 1e-8, None)

    corr = project_to_psd_correlation(similarity)
    q_vals = np.clip(np.linalg.eigvalsh(corr), 1e-8, None)
    q_vals = np.sort(q_vals)[::-1]
    profile = np.interp(
        np.linspace(0.0, len(q_vals) - 1, len(vals)),
        np.arange(len(q_vals)),
        q_vals,
    )
    profile = profile / (profile.mean() + EPS)
    local_rank = float(np.sum(vals) ** 2 / (np.sum(vals**2) + EPS))
    rank_penalty = np.clip(1.0 - local_rank / max(len(vals), 1), -0.5, 0.5)
    factors = 1.0 + lam * (profile - 1.0 + rank_penalty)
    factors = np.clip(factors, 0.7, 1.3)
    transform = vecs @ np.diag(np.sqrt(vals * factors)) @ vecs.T
    avg_scale = float(np.mean(factors))
    return 0.5 * (transform + transform.T), avg_scale
...
def letkf_update_weighted(
    ...
    for i in range(nx):
        ...
        w_pert, avg_scale = build_quantum_transform(pa_tilde=pa_tilde, similarity=similarity, lam=float(lam))

        x_mean_a = x_mean[i] + x_pert[:, i] @ w_mean
        x_pert_a = x_pert[:, i] @ w_pert
        ana[:, i] = x_mean_a + x_pert_a
        transform_scale_total += avg_scale
        transform_scale_steps += 1

    stats = {
        "avg_transform_scale": float(transform_scale_total / max(transform_scale_steps, 1)),
    }
    return ana, stats
...
def run_filter(
    ...
    stats_acc = {"avg_transform_scale": 0.0}
    stats_steps = 0
    ...
    if np.any(~np.isnan(obs[0])):
        ...
        analysis[0] = ensemble.mean(axis=0)
        if method == "quantum_transform":
            stats_acc["avg_transform_scale"] += step_stats["avg_transform_scale"]
            stats_steps += 1
    ...
    if stats_steps > 0:
        stats_acc["avg_transform_scale"] /= stats_steps
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验16：量子集合变换矩阵 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None]]:
    ...
    for lam in LAMBDAS:
        experiments.append(("quantum_transform", lam))
    return experiments
...
def main() -> None:
    ...
    for method, lam in experiments:
        ...
        for split, path in datasets:
            ...
            if method == "quantum_transform":
                rec[f"{split}_avg_transform_scale"] = stats["avg_transform_scale"]
    ...
    with (args.output_dir / "summary.csv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "train_mae",
                "train_count",
                "train_avg_transform_scale",
                "test_1_rmse",
                "test_1_mae",
                "test_1_count",
                "test_1_avg_transform_scale",
            ],
        )
        writer.writeheader()
