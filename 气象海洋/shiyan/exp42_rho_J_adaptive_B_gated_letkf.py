...
OUTPUT_DIR = ROOT / "exp42_rho_J_adaptive_B_gated_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "rho_J_adaptive_B_gated_letkf"
...
def compute_quantum_focus(quantum_weights: np.ndarray) -> float:
    probs = np.asarray(quantum_weights, dtype=float)
...
def compute_innovation_strength(innovation: np.ndarray) -> float:
    return float(np.tanh(np.mean(np.abs(innovation)) / (OBS_STD + EPS)))


def compute_j_confidence(
    coupling_strength: float,
    quantum_focus: float,
    innovation_strength: float,
) -> float:
    score = 0.2 + 0.45 * coupling_strength + 0.25 * quantum_focus + 0.1 * innovation_strength
    return float(np.clip(score, 0.15, 1.0))


def compute_alignment(signal: np.ndarray, local_mode: np.ndarray) -> float:
    alignment = abs(float(np.dot(signal, local_mode) / ((np.linalg.norm(signal) + EPS) * (np.linalg.norm(local_mode) + EPS))))
    return float(np.clip(alignment, 0.0, 1.0))


def compute_spread_penalty(signal: np.ndarray) -> float:
    spread_scale = float(np.std(signal) / (OBS_STD + EPS))
    penalty = 1.0 - 0.35 * np.tanh(spread_scale)
    return float(np.clip(penalty, 0.35, 1.0))


def compute_adaptive_b_lambda(
    quantum_signal: float,
    innovation_signal: float,
    alignment: float,
    spread_penalty: float,
) -> float:
    base = 0.0015 + 0.0015 * quantum_signal + 0.0015 * innovation_signal + 0.0010 * alignment
    lam = base * spread_penalty
    return float(np.clip(lam, 0.0015, 0.006))


def compute_b_confidence(
    alignment: float,
    quantum_signal: float,
    innovation_signal: float,
    spread_penalty: float,
) -> float:
    score = 0.15 + 0.35 * alignment + 0.2 * quantum_signal + 0.2 * innovation_signal + 0.1 * spread_penalty
    return float(np.clip(score, 0.1, 1.0))


def build_j_gated_state_signal(
    i: int,
    x_pert: np.ndarray,
    y_pert: np.ndarray,
    innovation: np.ndarray,
    quantum_raw: np.ndarray,
    lam_j: float,
) -> tuple[np.ndarray, np.ndarray, float, float, float]:
    quantum_weights = normalize_weights(quantum_raw, min_clip=0.1)
    ...
    state_signal = x_pert[:, i]
    signed_coupling = float(np.dot(state_signal, local_mode) / (np.linalg.norm(state_signal) + EPS))
    coupling_strength = float(np.clip(abs(signed_coupling), 0.0, 1.0))
    quantum_focus = compute_quantum_focus(quantum_weights)
    innovation_strength = compute_innovation_strength(innovation)
    j_confidence = compute_j_confidence(
        coupling_strength=coupling_strength,
        quantum_focus=quantum_focus,
        innovation_strength=innovation_strength,
    )

    j_mode = signed_coupling * local_mode
    ...
    quantum_signal = float(np.clip(np.mean(quantum_raw), 0.0, 1.0))
    return corrected, local_mode, correction_strength, j_confidence, quantum_signal


def build_b_gated_state_signal(
    ...
    innovation_strength = compute_innovation_strength(innovation)
    alignment = compute_alignment(j_corrected, local_mode)
    spread_penalty = compute_spread_penalty(j_corrected)
    lam_b_i = compute_adaptive_b_lambda(
        quantum_signal=quantum_signal,
        innovation_signal=innovation_strength,
        alignment=alignment,
        spread_penalty=spread_penalty,
    )
    b_confidence = compute_b_confidence(
        alignment=alignment,
        quantum_signal=quantum_signal,
        innovation_signal=innovation_strength,
        spread_penalty=spread_penalty,
    )
    signed_alignment = float(np.dot(j_corrected, local_mode) / (np.linalg.norm(j_corrected) + EPS))
    b_mode = signed_alignment * local_mode
    corrected = j_corrected + lam_b_i * b_confidence * b_mode
    corrected = corrected - corrected.mean()
    correction_strength = float(np.linalg.norm(corrected - j_corrected) / (np.linalg.norm(j_corrected) + EPS))
    return corrected, correction_strength, b_confidence
...
def letkf_update_weighted(
    ...
    j_correction_total = 0.0
    j_confidence_total = 0.0
    b_correction_total = 0.0
    b_confidence_total = 0.0
    correction_steps = 0
    ...
    if len(obs_idx_all) == 0:
        return ensemble.copy(), {
            "avg_j_correction_strength": 0.0,
            "avg_j_confidence": 0.0,
            "avg_b_correction_strength": 0.0,
            "avg_b_confidence": 0.0,
        }
    ...
    for i in range(nx):
        ...
        if method != "rho_j_adaptive_b_gated":
        ...
        j_corrected, local_mode, j_strength, j_confidence, quantum_signal = build_j_gated_state_signal(
            i=i,
            x_pert=x_pert,
            y_pert=y_pert,
        ...
        final_corrected, b_strength, b_confidence = build_b_gated_state_signal(
            j_corrected=j_corrected,
            local_mode=local_mode,
            innovation=innovation,
            quantum_signal=quantum_signal,
        )
        x_mean_a = x_mean[i] + final_corrected @ w_mean
        x_pert_a = final_corrected @ w_pert
        ana[:, i] = x_mean_a + x_pert_a

        j_correction_total += j_strength
        j_confidence_total += j_confidence
        b_correction_total += b_strength
        b_confidence_total += b_confidence
        correction_steps += 1

    stats = {
        "avg_j_correction_strength": float(j_correction_total / max(correction_steps, 1)),
        "avg_j_confidence": float(j_confidence_total / max(correction_steps, 1)),
        "avg_b_correction_strength": float(b_correction_total / max(correction_steps, 1)),
        "avg_b_confidence": float(b_confidence_total / max(correction_steps, 1)),
    }
    return ana, stats
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验42：rho+J+adaptive_B 双门控 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None, float | None]]:
    ...
    for lam_rho, lam_j in LAMBDA_COMBOS:
        experiments.append(("rho_j_adaptive_b_gated", lam_rho, lam_j))
    return experiments
    ...
    for method, lam_rho, lam_j in experiments:
        ...
        for split, path in datasets:
            ...
            if method == "rho_j_adaptive_b_gated":
                ...
                rec[f"{split}_avg_j_confidence"] = stats["avg_j_confidence"]
                rec[f"{split}_avg_b_correction_strength"] = stats["avg_b_correction_strength"]
                rec[f"{split}_avg_b_confidence"] = stats["avg_b_confidence"]
