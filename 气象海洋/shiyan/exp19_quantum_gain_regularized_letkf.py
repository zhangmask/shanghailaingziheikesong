...
OUTPUT_DIR = ROOT / "exp19_quantum_gain_regularized_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "quantum_gain_regularized_letkf"
...
def regularize_gain_row(gain_row: np.ndarray, similarity: np.ndarray, lam: float) -> tuple[np.ndarray, float]:
    corr = project_to_psd_correlation(similarity)
    degree = np.diag(np.sum(corr, axis=1))
    laplacian = degree - corr
    reg_mat = np.eye(corr.shape[0], dtype=float) + lam * laplacian
    gain_reg = stable_inverse(reg_mat) @ gain_row
    smoothness = float(gain_reg @ laplacian @ gain_reg / (gain_reg @ gain_reg + EPS))
    return gain_reg, smoothness
...
def letkf_update_weighted(
    ...
    if len(obs_idx_all) == 0:
        return ensemble.copy(), {"avg_gain_smoothness": 0.0}

    global_obs_similarity = None
    if method == "quantum_gain_regularized":
    ...
    for i in range(nx):
        ...
        pa_tilde = stable_inverse(c_mat)
        w_pert = sym_sqrt((nens - 1) * pa_tilde)
        classical_gain_row = x_pert[:, i] @ pa_tilde @ y_pert @ r_inv

        local_pos = np.searchsorted(obs_idx_all, local_obs_idx)
        similarity = global_obs_similarity[np.ix_(local_pos, local_pos)]
        gain_reg, smoothness = regularize_gain_row(classical_gain_row, similarity, float(lam))

        x_mean_a = x_mean[i] + gain_reg @ innovation
        x_pert_a = x_pert[:, i] @ w_pert
        ana[:, i] = x_mean_a + x_pert_a
        smoothness_total += smoothness
        smoothness_steps += 1

    stats = {
        "avg_gain_smoothness": float(smoothness_total / max(smoothness_steps, 1)),
    }
    return ana, stats
...
def run_filter(
    ...
    stats_acc = {"avg_gain_smoothness": 0.0}
    stats_steps = 0
    ...
    if np.any(~np.isnan(obs[0])):
        ...
        analysis[0] = ensemble.mean(axis=0)
        if method == "quantum_gain_regularized":
            stats_acc["avg_gain_smoothness"] += step_stats["avg_gain_smoothness"]
            stats_steps += 1
    ...
    if stats_steps > 0:
        stats_acc["avg_gain_smoothness"] /= stats_steps
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验19：量子增益正则化 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None]]:
    ...
    for lam in LAMBDAS:
        experiments.append(("quantum_gain_regularized", lam))
    return experiments
...
def main() -> None:
    ...
    for method, lam in experiments:
        ...
        for split, path in datasets:
            ...
            if method == "quantum_gain_regularized":
                rec[f"{split}_avg_gain_smoothness"] = stats["avg_gain_smoothness"]
    ...
    with (args.output_dir / "summary.csv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "train_mae",
                "train_count",
                "train_avg_gain_smoothness",
                "test_1_rmse",
                "test_1_mae",
                "test_1_count",
                "test_1_avg_gain_smoothness",
            ],
        )
        writer.writeheader()
