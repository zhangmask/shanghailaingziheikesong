...
OUTPUT_DIR = ROOT / "exp17_quantum_B_correction_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "quantum_B_correction_letkf"
...
def build_b_corrected_state_signal(
    ...
    local_mode_norm = np.linalg.norm(local_mode) + EPS
    local_mode = local_mode / local_mode_norm

    state_signal = x_pert[:, i]
    coupling = float(np.dot(state_signal, local_mode) / (np.linalg.norm(state_signal) + EPS))
    corrected = state_signal + lam * coupling * local_mode
...
def letkf_update_weighted(
    ...
    for i in range(nx):
        ...
        if method != "quantum_b_correction":
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验17：量子背景协方差修正 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None]]:
    ...
    for lam in LAMBDAS:
        experiments.append(("quantum_b_correction", lam))
    return experiments
