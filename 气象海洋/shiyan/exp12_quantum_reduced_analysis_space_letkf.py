...
OUTPUT_DIR = ROOT / "exp12_quantum_reduced_analysis_space_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "quantum_reduced_analysis_space_letkf"
...
def build_reduction_basis(similarity: np.ndarray, reduced_dim: int) -> np.ndarray:
    psd = project_to_psd(similarity)
    vals, vecs = np.linalg.eigh(psd)
    order = np.argsort(vals)[::-1]
    rank = min(reduced_dim, similarity.shape[0])
    chosen = order[:rank]
    basis = vecs[:, chosen]
...
def letkf_update_weighted(
    ...
    if len(obs_idx_all) == 0:
        return ensemble.copy(), {"avg_reduced_dim": 0.0}

    global_obs_similarity = None
    if method == "quantum_reduced_analysis_space":
    ...
    for i in range(nx):
        ...
        basis = build_reduction_basis(similarity=similarity, reduced_dim=int(lam))
        z_pert = y_pert @ basis
        z_innovation = basis.T @ innovation
    ...
    stats = {
        "avg_reduced_dim": float(reduced_dim_total / max(reduced_steps, 1)),
    }
    return ana, stats
...
def run_filter(
    ...
    group_stats_accumulator = {"avg_reduced_dim": 0.0}
    group_stats_steps = 0
    ...
    if np.any(~np.isnan(obs[0])):
        ...
        analysis[0] = ensemble.mean(axis=0)
        if method == "quantum_reduced_analysis_space":
            group_stats_accumulator["avg_reduced_dim"] += float(step_stats["avg_reduced_dim"])
            group_stats_steps += 1
    ...
    if group_stats_steps > 0:
        group_stats_accumulator["avg_reduced_dim"] /= group_stats_steps
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验12：量子降阶分析空间 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None]]:
    ...
    for reduced_dim in REDUCED_DIMS:
        experiments.append(("quantum_reduced_analysis_space", reduced_dim))
    return experiments
...
def main() -> None:
    ...
    for method, lam in experiments:
        ...
        for split, path in datasets:
            ...
            if method == "quantum_reduced_analysis_space":
                rec[f"{split}_avg_reduced_dim"] = group_stats["avg_reduced_dim"]
    ...
    with (args.output_dir / "summary.csv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "train_mae",
                "train_count",
                "train_avg_reduced_dim",
                "test_1_rmse",
                "test_1_mae",
                "test_1_count",
                "test_1_avg_reduced_dim",
            ],
        )
        writer.writeheader()
