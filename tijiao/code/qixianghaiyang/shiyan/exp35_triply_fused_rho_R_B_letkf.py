...
OUTPUT_DIR = ROOT / "exp35_triply_fused_rho_R_B_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "triply_fused_rho_R_B_letkf"
...
def letkf_update_weighted(
    ...
    if method == "triply_fused_rho_r_b":
        obs_fill = np.nan_to_num(obs_vec, nan=0.0)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验35：rho-R-B 三元弱融合 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None, float | None, float | None]]:
    experiments: list[tuple[str, float | None, float | None, float | None]] = [
        ("fixed", None, None, None),
        ("corr", None, None, None),
    ]
    for lam_rho, lam_r, lam_b in LAMBDA_COMBOS:
        experiments.append(("triply_fused_rho_r_b", lam_rho, lam_r, lam_b))
    return experiments


def result_file_stem(method: str, lam_rho: float | None, lam_r: float | None, lam_b: float | None) -> str:
    if lam_rho is None and lam_r is None and lam_b is None:
        return method
    return (
        f"{method}_rho_{str(lam_rho).replace('.', '_')}_"
        f"r_{str(lam_r).replace('.', '_')}_"
...
def main() -> None:
    ...
    for method, lam_rho, lam_r, lam_b in experiments:
        ...
        for split, path in datasets:
            ...
            out_path = args.output_dir / f"{split}_{result_file_stem(method, lam_rho, lam_r, lam_b)}.csv"
