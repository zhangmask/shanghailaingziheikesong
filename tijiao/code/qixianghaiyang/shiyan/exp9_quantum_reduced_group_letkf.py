...
OUTPUT_DIR = ROOT / "exp9_quantum_reduced_group_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "quantum_reduced_group_letkf"
...
def grouped_local_analysis(
    ...
    infl: float,
    i: int,
    group_obs_idx: np.ndarray,
) -> np.ndarray:
    nens = ensemble.shape[0]
    y_local = obs_vec[group_obs_idx]
    y_ens = ensemble[:, group_obs_idx]
    y_mean = y_ens.mean(axis=0)
    y_pert = y_ens - y_mean
    innovation = y_local - y_mean
    weights = np.ones(len(group_obs_idx), dtype=float)
...
def select_group_representatives(
    ...
    group_sim = similarity[np.ix_(group, group)]
    cohesion = group_sim.mean(axis=1)
    medoid_local = int(np.argmax(cohesion))
    selected_local = [medoid_local]

    # 保留组中心，再补一个与中心差异更明显的代表点，做最小降阶。
    while len(selected_local) < max_representatives:
        best_idx = -1
        best_score = -float("inf")
        for candidate_local in range(len(group)):
            if candidate_local in selected_local:
                continue
            diversity = -float(np.mean([group_sim[candidate_local, chosen] for chosen in selected_local]))
            score = float(0.7 * cohesion[candidate_local] + 0.3 * diversity)
            if score > best_score:
                best_score = score
                best_idx = candidate_local
        if best_idx == -1:
            break
        selected_local.append(best_idx)

    return group[np.array(selected_local, dtype=int)]


def compute_group_consistency(similarity: np.ndarray, group: np.ndarray) -> float:
    if len(group) <= 1:
        return 1.0
    group_sim = similarity[np.ix_(group, group)]
    ...
    if not np.any(mask):
        return 1.0
    return float(np.clip(group_sim[mask].mean(), 0.2, 1.0))
...
def letkf_update_weighted(
    ...
    if method == "quantum_reduced_group_update":
    ...
    for i in range(nx):
        ...
        groups = partition_obs_groups(similarity=similarity, threshold=GROUP_THRESHOLD, max_groups=MAX_GROUPS)
        grouped_members = [local_obs_idx[group] for group in groups if len(group) > 0]
        if not grouped_members:
            grouped_members = [local_obs_idx]

        group_analysis = np.zeros(nens, dtype=float)
        total_weight = 0.0
        for group_obs_idx in grouped_members:
            local_group_pos = np.searchsorted(local_obs_idx, group_obs_idx)
            representatives = select_group_representatives(
                similarity=similarity,
                group=local_group_pos,
                max_representatives=MAX_REPRESENTATIVES,
            )
            rep_obs_idx = local_obs_idx[representatives]
            consistency = compute_group_consistency(similarity=similarity, group=local_group_pos)
            compression_weight = float(len(group_obs_idx)) / max(len(rep_obs_idx), 1)
            group_weight = float(len(group_obs_idx)) * consistency * compression_weight
            group_analysis += group_weight * grouped_local_analysis(
                x_mean=x_mean,
                x_pert=x_pert,
                ensemble=ensemble,
                obs_vec=obs_vec,
                ...
                infl=infl,
                i=i,
                group_obs_idx=rep_obs_idx,
            )
            total_weight += group_weight
            group_size_total += int(len(group_obs_idx))
        ana[:, i] = group_analysis / max(total_weight, 1.0)

        group_count_total += len(grouped_members)
        group_eval_count += 1
        max_group_count = max(max_group_count, len(grouped_members))

    stats = {
        "avg_group_count": float(group_count_total / max(group_eval_count, 1)),
        "avg_group_size": float(group_size_total / max(group_count_total, 1)),
...
def run_filter(
    ...
    group_stats_accumulator = {"avg_group_count": 0.0, "avg_group_size": 0.0, "max_group_count": 0}
    group_stats_steps = 0
    ...
    if np.any(~np.isnan(obs[0])):
        ...
        analysis[0] = ensemble.mean(axis=0)
        if method == "quantum_reduced_group_update":
            group_stats_accumulator["avg_group_count"] += float(step_stats["avg_group_count"])
            group_stats_accumulator["avg_group_size"] += float(step_stats["avg_group_size"])
            group_stats_accumulator["max_group_count"] = max(
                int(group_stats_accumulator["max_group_count"]),
                int(step_stats["max_group_count"]),
            )
            group_stats_steps += 1
    ...
    if group_stats_steps > 0:
        group_stats_accumulator["avg_group_count"] /= group_stats_steps
        group_stats_accumulator["avg_group_size"] /= group_stats_steps
    return analysis, {"rmse": rmse, "mae": mae, "count": int(mask.sum())}, group_stats_accumulator
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验9：量子核表示层分组降阶 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None]]:
    return [("fixed", None), ("corr", None), ("quantum_reduced_group_update", None)]
...
def main() -> None:
    ...
    for method, lam in experiments:
        ...
        for split, path in datasets:
            ...
            analysis, metrics, group_stats = run_filter(obs, truth, method=method, lam=lam)
            ...
            if method == "quantum_reduced_group_update":
                rec[f"{split}_avg_group_count"] = group_stats["avg_group_count"]
                rec[f"{split}_avg_group_size"] = group_stats["avg_group_size"]
                rec[f"{split}_max_group_count"] = group_stats["max_group_count"]
    ...
    with (args.output_dir / "summary.csv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "train_mae",
                "train_count",
                "train_avg_group_count",
                "train_avg_group_size",
                "train_max_group_count",
                "test_1_rmse",
                "test_1_mae",
                "test_1_count",
                "test_1_avg_group_count",
                "test_1_avg_group_size",
                "test_1_max_group_count",
            ],
        )
        writer.writeheader()
