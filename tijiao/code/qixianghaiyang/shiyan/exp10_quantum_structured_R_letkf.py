...
OUTPUT_DIR = ROOT / "exp10_quantum_structured_R_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "quantum_structured_R_letkf"
...
def letkf_update_weighted(
    ...
    if method == "quantum_structured_r":
    ...
    for i in range(nx):
        ...
        structured_r = build_structured_r(obs_var=obs_var, similarity=similarity, lam=float(lam))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验10：量子结构化观测误差 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None]]:
    ...
    for lam in LAMBDAS:
        experiments.append(("quantum_structured_r", lam))
    return experiments
