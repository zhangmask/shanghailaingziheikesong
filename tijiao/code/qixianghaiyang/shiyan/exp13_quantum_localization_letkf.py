...
OUTPUT_DIR = ROOT / "exp13_quantum_localization_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "quantum_localization_letkf"
...
def letkf_update_weighted(
    ...
    for i in range(nx):
        ...
        if method != "quantum_localization":
        ...
        weights = normalize_weights(dist_prior * ((1.0 - float(lam)) + float(lam) * quantum_raw))
        r_inv = np.diag(weights / obs_var)
        c_mat = ((nens - 1) / infl) * np.eye(nens) + y_pert @ r_inv @ y_pert.T
        pa_tilde = stable_inverse(c_mat)
        w_mean = pa_tilde @ y_pert @ r_inv @ innovation
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验13：量子局地化核 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None]]:
    ...
    for lam in LAMBDAS:
        experiments.append(("quantum_localization", lam))
    return experiments
