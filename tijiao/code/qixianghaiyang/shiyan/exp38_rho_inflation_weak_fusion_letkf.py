...
OUTPUT_DIR = ROOT / "exp38_rho_inflation_weak_fusion_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "rho_inflation_weak_fusion_letkf"
...
def compute_local_inflation(
    ...
    combined = np.clip(0.5 * quantum_signal + 0.25 * innovation_signal + 0.25 * spread_signal, 0.0, 1.5)
    factor = 1.0 + lam_infl * (combined - 0.75)
    infl_local = float(np.clip(infl * factor, 0.95, 1.20))
    return infl_local, infl_local
...
def letkf_update_weighted(
    ...
    if len(obs_idx_all) == 0:
        return ensemble.copy(), {"avg_inflation": 0.0}
    ...
    for i in range(nx):
        ...
        if method != "rho_inflation_weak_fusion":
        ...
        infl_local, infl_value = compute_local_inflation(
            i=i,
            x_mean=x_mean,
            obs_vec=obs_vec,
            local_obs_idx=local_obs_idx,
            ...
            y_pert=y_pert,
            infl=infl,
            lam_infl=float(lam_infl),
        )
        r_inv = np.diag(weights / obs_var)
        c_mat = ((nens - 1) / infl_local) * np.eye(nens) + y_pert @ r_inv @ y_pert.T
        pa_tilde = stable_inverse(c_mat)
        w_mean = pa_tilde @ y_pert @ r_inv @ innovation
        w_pert = sym_sqrt((nens - 1) * pa_tilde)
        x_mean_a = x_mean[i] + x_pert[:, i] @ w_mean
        x_pert_a = x_pert[:, i] @ w_pert
        ana[:, i] = x_mean_a + x_pert_a
        infl_total += infl_value
        infl_steps += 1

    stats = {
        "avg_inflation": float(infl_total / max(infl_steps, 1)),
    }
    return ana, stats
...
def run_filter(
    ...
    if np.any(~np.isnan(obs[0])):
        ...
        analysis[0] = ensemble.mean(axis=0)
        if method == "rho_inflation_weak_fusion":
            stats_acc["avg_inflation"] += step_stats["avg_inflation"]
            stats_steps += 1
    ...
    if stats_steps > 0:
        stats_acc["avg_inflation"] /= stats_steps
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验38：rho-inflation 弱融合 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None, float | None]]:
    ...
    for lam_rho, lam_infl in LAMBDA_COMBOS:
        experiments.append(("rho_inflation_weak_fusion", lam_rho, lam_infl))
    return experiments


def result_file_stem(method: str, lam_rho: float | None, lam_infl: float | None) -> str:
    if lam_rho is None and lam_infl is None:
        return method
    return (
        f"{method}_rho_{str(lam_rho).replace('.', '_')}_"
        f"infl_{str(lam_infl).replace('.', '_')}"
    )
...
def main() -> None:
    ...
    for method, lam_rho, lam_infl in experiments:
        rec: dict[str, float | int | str | None] = {
            "method": method,
            "lambda_rho": lam_rho,
            "lambda_infl": lam_infl,
            "support_radius": SUPPORT_RADIUS,
        }
        for split, path in datasets:
            ...
            out_path = args.output_dir / f"{split}_{result_file_stem(method, lam_rho, lam_infl)}.csv"
            ...
            if method == "rho_inflation_weak_fusion":
                rec[f"{split}_avg_inflation"] = stats["avg_inflation"]
