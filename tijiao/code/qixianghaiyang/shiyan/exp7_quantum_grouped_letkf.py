...
OUTPUT_DIR = ROOT / "exp7_quantum_grouped_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "quantum_grouped_letkf"
...
def letkf_update_weighted(
    ...
    if method == "quantum_grouped_update":
    ...
    for i in range(nx):
        ...
        for group_obs_idx in grouped_members:
            group_weight = float(len(group_obs_idx))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验7：量子核表示层分组接入 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None]]:
    return [("fixed", None), ("corr", None), ("quantum_grouped_update", None)]
