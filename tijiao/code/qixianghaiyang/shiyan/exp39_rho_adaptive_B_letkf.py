...
OUTPUT_DIR = ROOT / "exp39_rho_adaptive_B_outputs"
REMOTE_BASE = Path("/home/infra/qda_competition")
REMOTE_OUTPUT = REMOTE_BASE / "experiments" / "rho_adaptive_B_letkf"
...
def compute_adaptive_b_lambda(quantum_signal: float, innovation_signal: float) -> float:
    lam = 0.002 + 0.003 * quantum_signal + 0.003 * innovation_signal
    return float(np.clip(lam, 0.002, 0.008))


def build_b_corrected_state_signal(
    i: int,
    x_mean: np.ndarray,
    x_pert: np.ndarray,
    y_pert: np.ndarray,
    ...
    corrected = state_signal + lam_b * coupling * local_mode
...
def letkf_update_weighted(
    ...
    if len(obs_idx_all) == 0:
        return ensemble.copy(), {"avg_b_correction_strength": 0.0}
    ...
    for i in range(nx):
        ...
        lam_b_i = compute_adaptive_b_lambda(quantum_signal, innovation_signal)
        corrected_signal, strength = build_b_corrected_state_signal(
            i=i,
            x_mean=x_mean,
            x_pert=x_pert,
            y_pert=y_pert,
    ...
    stats = {
        "avg_b_correction_strength": float(correction_total / max(correction_steps, 1)),
    }
    return ana, stats
...
def run_filter(
    ...
    stats_acc = {"avg_b_correction_strength": 0.0}
    stats_steps = 0
    ...
    if np.any(~np.isnan(obs[0])):
        ...
        analysis[0] = ensemble.mean(axis=0)
        if method == "rho_adaptive_b":
            stats_acc["avg_b_correction_strength"] += step_stats["avg_b_correction_strength"]
            stats_steps += 1
    ...
    if stats_steps > 0:
        stats_acc["avg_b_correction_strength"] /= stats_steps
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="实验39：rho-adaptive-B 融合 LETKF 验证")
...
def build_experiments() -> list[tuple[str, float | None, float | None]]:
    ...
    for lam_rho, lam_b in LAMBDA_COMBOS:
        experiments.append(("rho_adaptive_b", lam_rho, lam_b))
    return experiments


def result_file_stem(method: str, lam_rho: float | None, lam_b: float | None) -> str:
    if lam_rho is None and lam_b is None:
        return method
    return (
        f"{method}_rho_{str(lam_rho).replace('.', '_')}_"
        f"b_{str(lam_b).replace('.', '_')}"
    )
...
def main() -> None:
    ...
    for method, lam_rho, lam_b in experiments:
        ...
        for split, path in datasets:
            ...
            out_path = args.output_dir / f"{split}_{result_file_stem(method, lam_rho, lam_b)}.csv"
