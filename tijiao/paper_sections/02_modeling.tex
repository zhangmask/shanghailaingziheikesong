...
\end{equation}
其中 $H$ 为单位观测算子，观测噪声服从高斯分布。设集合规模为 $N_e$，第 $i$ 个状态维度对应的集合扰动向量记为 $\tilde{x}_i\in \mathbb{R}^{N_e}$，局地观测集合记为 $\mathcal{O}_i$，对应的观测扰动矩阵记为 $Y_i\in\mathbb{R}^{N_e\times |\mathcal{O}_i|}$。在经典 LETKF 中，若局地观测逆协方差为 $R_i^{-1}$，则权重空间的分析矩阵写为。该写法与 LETKF 的局地权重空间更新以及集合卡尔曼方法的均值场视角是一致的\cite{hunt2007letkf,calvello2025enkf}.
\begin{equation}
...
这里的“局地窗口”并不是单纯为了降维而做的技术处理，而是整个建模思路的核心。Lorenz96 本身具有明显的局地耦合结构，单个状态维度的演化主要受邻近维度影响。因此，如果把整个 $40$ 维状态一次性量子化，不仅资源上不可行，也会掩盖问题本来就具有的局地结构特征。相反，把问题拆解为多个重叠局地窗口后，每个窗口都可以被解释为一个小尺度的“状态模式识别”任务，量子模块在这里更容易发挥几何表示能力。这种做法与近年的物理驱动局部化思路在精神上是一致的：都强调局地结构不应完全由经验函数硬编码，而应由状态相关信息决定\cite{sarp2025localization}.

从建模角度看，本文的关键不是把经典变量机械映射到量子比特上，而是先完成一个问题重述：原问题是“如何在全局状态空间中更新分析场”，重述后变为“如何在每个局地窗口中识别哪些观测模式与当前状态更匹配”。量子特征映射承担的就是这个局地模式识别任务。这样一来，量子模块的输出不再是直接替代分析值，而是为后续的经典更新提供一张局地相似度地图。这一点与量子生成器核和量子切线核文献中强调的“核函数反映嵌入几何而不仅是静态相似度”的思路是一致的\cite{altmann2025qgk,qtk2024}。
